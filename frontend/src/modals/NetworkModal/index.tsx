  ...
  return (
    <Modal isOpen={networkModalOpen} onDismiss={toggleNetworkModal} maxWidth={672}>
      ...
      <div className="mb-6 text-lg text-primary">
        You are currently browsing <span className="font-bold text-pink">Lux Town</span>
        <br /> on the <span className="font-bold text-blue">{NETWORK_LABEL[chainId]}</span> network
      </div>

      <div className="grid grid-flow-row-dense grid-cols-1 gap-5 overflow-y-auto md:grid-cols-2">
        {SUPPORTED_NETWORK_CHAIN_ID.map((key: ChainId, i: number) => {
          if (chainId === key) {
            return (
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                console.log('NetworkModal.onClick', { chainId: key })
                const params = SUPPORTED_NETWORKS[key]
                if (DEFAULT_METAMASK_CHAIN_ID.includes(chainId)) {
